...
'''
The following was not used for the johnson topics analysis.
### segmenter
...
infolder = join(wdir, "data/letters-leipzig-txt")
outfolder = join(wdir, "data/letters-leipzig-tagged")
tagger = "/home/ulrike/Programs/treetagger/cmd/tree-tagger-german"
prepare.call_treetagger(infolder, outfolder, tagger) 

### make_lemmatext
### Extract selected lemmata from tagged text.
inpath = join(wdir, "data/letters-leipzig-tagged", "*.trt")
modes = ["NNE", "N", "NNEVAdj", "NVAdj"]
for mode in modes:
	outfolder = join(wdir, "data/letters-leipzig-lemmata-" + mode)
	mode = "de" + mode
